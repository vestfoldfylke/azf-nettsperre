...
app.http("updateBlock", {
  ...
  handler: async (request, _context) => {
    ...
    response = {
      ...lastItem,
      blockedGroup: {
        displayName: body.blockedGroup.displayName
      }
    };
    ...
    try {
      ...
      //     // Remove members from the group since the block is already active and remove from the array of students
      //     logger.info("{logPrefix} - Number of students to remove: {StudentsToRemoveCount}", logPrefix, lastItem.studentsToRemove.length)
      //     const groupId = await body.typeBlock.groupId
      ...
      if (isChanged) {
        ...
        await mongoClient
          .db(mongoDB.dbName)
          .collection(mongoDB.blocksCollection)
          .updateOne({ _id: id }, { $push: { updated: lastItem } });
      }
